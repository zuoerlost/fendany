    ...
    public void write(byte[] data, int offset, int length) throws IOException {
        if (offset == 0) {
            ...
            if (writtenLength != length) {
                throw new IOException("【UnixSocketOutputStream】【send】: length is " + length + " but only wrote " + writtenLength);
            }
        } else {
            throw new IOException("【UnixSocketOutputStream】【send】: offset not supported");
        }
    }
